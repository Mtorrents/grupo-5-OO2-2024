package com.unla.ghsicilianotfi.helpers;

public class ViewRouteHelper {
	public static final String USER_LOGIN = "user/login"; // Ajusta el valor según la ubicación de tu plantilla
    public static final String USER_LOGOUT = "user/logout"; // Ajusta el valor según la ubicación de tu plantilla
    public final static String PRODUCTO_INDEX = "producto/index";
    public final static String PRODUCTO_NEW = "producto/new";
    public final static String PRODUCTO_UPDATE = "producto/update";
    public final static String PRODUCTO_ROOT = "/producto";
}